;*****************************************************************************
;
;    dsp3210mpeg.device - mpeg.device for DSP3210 on AA3000(+) Amigas
;    Copyright (C) 2024 Wrangler
;
...
; This will automatically remove the device from the system
; after closing (EXPUNGE = 1). Otherwise set to 0.
;;EXPUNGE		SET	1
...
; Here, define device global data (Library ptr, etc.)
...
****************************************************************************
	...
	section	mycode,code
        ...
        moveq #-1,d0		;don't try to run this!
        rts                                
...
InitTable	dc.w	RTC_MATCHWORD
		...
		dc.b	"$VER: "
IDString	dc.b	"dsp3210mpeg.device v1.4 (17.07.2025) © Wrangler 2025",0

_DevName	dc.b	"dsp3210mpeg.device",0
...
*
* This is a short description of the MPEG device. This can be
* queried by the application.
	cnop	0,4
BoardDesc
	dc.b	"MPEG audio decoder for the DSP3210."
	dc.b	" supports MPEG-1 audio Layer II/III."
	dc.b	" Copyright © Wrangler 2025."
endtext
...
****************************************************************************
*
* Open is called when the device is opened
*
...
****************************************************************************
*
* Close is called when the device is closed
*
* -> a1=ioreq, a6=device
* <- d0=seglist oder 0
*
Close__
	...
	moveq	#0,d0
	move.l	a3,d1
	beq.b	.close_end		; unit is already closed
...
Expunge__
	...
	move.l	a6,a5			; device ptr
	move.l	md_SysLib(a5),a6	; ExecBase to a6
	tst	LIB_OPENCNT(a5)
	beq.b	.exp_ok
...
.exp_ok	move.l	md_SegList(a5),d2	; pointer to segment list
	move.l	a5,a1			; device
	jsr	-252(a6)		;CALL Remove Device from list


	move.l	a5,a1			; device
	moveq	#0,d0
	move	LIB_NEGSIZE(a5),d0
	sub.l	d0,a1			; Beginning of memory for device
	add	LIB_POSSIZE(a5),d0	; Determine the length of the device
	jsr	-210(a6)		;CALL FreeMem Free up memory
...
PerformIO__
	moveq	#0,d0
	move.b	d0,IO_ERROR(a1)		; no error
...
*
* If successful, AbortIO returns IOERR_ABORTED in IO_ERROR and zero in D0
*
...
*
TermIO__
	btst	#IOB_QUICK,IO_FLAGS(a1)	; if quick, then no ReplyMsg
	bne.b   .termio_end
	move.l	a6,-(a7)
...
****************************************************************************
*
* Sets the current system clock reference
*
* -> a0=unit, d0=time
* <- d0=succes
*
...
GetSCR__
	...
	rts
*
* Gives the current system clock reference
*
* -> a0=unit
...
****************************************************************************
* Begin the functions of the device commands
*
* Parameter:
* -> a1=iorequest, a3=unitptr, a6=device
*
...
*
* Stop the player and cancel all requests.
*
CMD_Flush__
	movem.l	d1-d7/a0-a6,-(a7)
	move.l	a3,-(a7)		;struct devunit*
...
*
* In this device the playback hardware is started via the Write function.
*
* INPUTS:
...
*
* INPUTS:
*   iomr_StreamType  - MPEGSTREAM_AUDIO or MPEGSTREAM_SYSTEM
*                      depending on stream type.
*   iomr_Arg1        - Pause-mode on: io_Arg not equal to 0
*                      Pause-mode off: io_Arg equal to 0
*
MPEG_Pause
	movem.l	d1-d7/a0-a6,-(a7)
...
*
* INPUTS:
*   IO_DATA - pointer to MPEGDevInfo structure
*
MPEG_GetDevInfo
	movem.l	a0-a3,-(a7)
...
*
* INPUTS:
*   IO_DATA   - pointer to MPEGAudioParams structure.
...
*
* INPUTS:
*   IO_DATA   - pointer to buffer with data
*   IO_LENGTH - number of bytes in the buffer
*
CMD_Write__
	movem.l	d1-d7/a0-a6,-(a7)
